...
namespace SystemSoftware.Common
{
    public static class Helpers
    {
        ...
        /// </summary>
        public static void ParseCondition(Macro te, string str, out int first, out int second, out string sign)
        {
            string[] arr;
            first = 0;
            second = 0;
            ...
            foreach (string sgn in ComparisonSigns)
            {
                if ((arr = str.Split(new string[] { sgn }, StringSplitOptions.None)).Length > 1)
                {
                    ...
                    else if (te.Dict.TryGetValue(arr[0], out first)) { }
                    ...
                    else if (te.Dict.TryGetValue(arr[1], out second)) { }
        ...
        public static void PushConditionArgs(string str, Macro te)
        {
            int first, second;
            string sign;
            Helpers.ParseCondition(te, str, out first, out second, out sign);
            string[] arr;
        ...
        /// <summary> Сравнение
        /// </summary>
        public static bool Compare(string str, Macro te)
        {
            int first;
            int second;
            string sign;
            ParseCondition(te, str, out first, out second, out sign);
            switch (sign)
            {
                case ">":
                    return first > second;
