    ...
    public class Processor
    {
        ...
        public void FirstRunStep(CodeEntity se, Macro te)
        {
            ...
            else if (operation == "SET" && _macroCount == 0)
            {
                ...
                if (VariablesStorage.IsInVariablesStorage(operands[0]))
                {
                    VariablesStorage.Find(se.Operands[0]).Value = Int32.Parse(se.Operands[1]);
                }
                if (te.Parameters != null && te.Parameters.Any(e => e.Name == operands[0].Trim()))
                {
                    te.Dict[se.Operands[0]] = Int32.Parse(se.Operands[1]);
                }
            }
        ...
        private List<CodeEntity> ProcessMacroParams(Macro macro, IEnumerable<string> passedParams)
        {
            ...
            macro.Dict = new Dictionary<string, int>();
            ...
            Action ParsePositionedParams = delegate ()
            {
                ...
                for (int i = 0; i < localFirstParamIndex; i++)
                {
                    ...
                    try
                    {
                        macro.Dict.Add(macro.Parameters[i].Name, curerntParamValue);
                    }
                    catch (Exception)
                    {
                        throw new CustomException(string.Format(ProcessorErrorMessages.MacroParameterIsEmptyVariable, currentParam));
            ...
            foreach (var sourceLine in processedBody)
            {
                    ...
                    if (sourceLine.Operands.Count > 0)
                    {
                        foreach (var sign in Helpers.ComparisonSigns)
                        {
                            ...
                            if (t.Length == 2)
                            {
                                ...
                                {
                                    //t[0] = macro.Dict[t[0].Trim()].ToString();
                                }
                                if (macro.Parameters.Any(e => e.Name == t[1].Trim()))
                                {
                                    //t[1] = macro.Dict[t[1].Trim()].ToString();
                                }
                ...
                else
                {
                    for (int i = 0; i < sourceLine.Operands.Count; i++)
                    {
                        ...
                        if (macro.Dict.Keys.Contains(currentOperand))
                        {
                            sourceLine.Operands[i] = macro.Dict[currentOperand].ToString();
                        }
            ...
            foreach (var se in processedBody)
            {
                if (se.Operation != "IF" && se.Operation != "WHILE" && se.Operation != "INC" && se.Operation != "SET")
                {
                    for (int i = 0; i < se.Operands.Count; i++)
                    {
                        if (macro.Dict.Keys.Contains(se.Operands[i]))
                        {
                            se.Operands[i] = macro.Dict[se.Operands[i]].ToString();
                        }
                    }
                }
            }
